...
impl ForumMinimal {
    ...
    pub fn compute_forum(&mut self, output0: Rc<RefCell<Vec<isize>>>) -> Rc<RefCell<InputSession<u64, (u64, Persisted), isize>>> {
            ...
            worker.dataflow(|scope| {
                ...
                manages
                    .filter(move |(_id, persisted)| {
                        if let Persisted::Post { .. } = persisted { true } else { false }
                    })
                    .count()
                    .inspect(move |(((_id, persisted), count), _time, _diff)| output1.borrow_mut().push(*count));
        ...
        let input0 : Rc<RefCell<InputSession<u64, (u64, Persisted), isize>>> = Rc::new(RefCell::new(input));
        let input1 = input0.clone();

        input0.borrow_mut().insert((10u64, Persisted::Post { title: "asdf".into(), user_id: 20, likes: 0 }));
